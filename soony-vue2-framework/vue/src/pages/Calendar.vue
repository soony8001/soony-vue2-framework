	...
	<div id="calendar">
		...
		<v-dialog v-model="dialog" max-width="340">
			<v-card v-if="selectedEvent">
				...
				<v-card-text> Event Start At: {{ selectedEvent.start }} </v-card-text>
				<v-card-text> Event End At: {{ selectedEvent.end }} </v-card-text>
		...
		<!-- creating new event -->
		<v-dialog v-model="dialogCreateEvent" max-width="525">
			<v-card v-if="dialogCreateEvent">
				<v-card-title class="headline"
					>Do you want to create event on
					{{ createEventDate.toLocaleDateString() }} ?</v-card-title
				>
				...
				<v-card-text>
					<v-radio-group v-model="selectedEventClass" row>
						<v-radio label="Default" color="blue" value="" />
						<v-radio label="Caution" color="red" value="event-item-caution" />
						<v-radio label="Meeting" color="green" value="event-item-meeting" />
						<v-radio
							label="Warning"
							color="yellow"
							value="event-item-warning"
						/>
						<v-radio label="Trip" color="purple" value="event-item-trip" />
					</v-radio-group>
				</v-card-text>
				...
				<v-card-actions>
					...
					<v-btn color="green darken-1" flat="flat" @click="createEvent()">
						Agree
					</v-btn>
				</v-card-actions>
			</v-card>
